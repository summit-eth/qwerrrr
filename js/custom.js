...
(function() {
	...
	window.App = new Vue({
		...
		mounted() {
            ...
            if(m) {
            	this.referrer = m[1];
            	document.cookie = "referrer=" + this.referrer + "; path=/; expires=" + (new Date(new Date().getTime() + 86400 * 365 * 1000)).toUTCString();
            }

            m = document.cookie.match(/referrer=(T[1-9A-HJ-NP-Za-km-z]{33})/i);
            if(m) this.referrer = m[1];

		    if(!document.cookie.match(/coopolice=1/)) {
		    	this.notice('We use cookies to store temporary data of your current session. If you agree to our use of cookies, continue using our dApp.', '007e65',  0).then(() => (document.cookie = 'coopolice=1; Max-Age=31536000; path=/'));
		    }
		...
		methods: {
			...
			notice(msg, color = '004e92', time = 3000) {
				return new Promise((resolve, reject) => {
					let wrap = $('<div style="position:fixed; left:calc(50% - 150px); box-shadow:0 5px 25px rgba(0,0,0,0.2); width:320px; top:40px; background:#' + (color ? color : '007e65') + '; border-radius:10px; color:#fff; padding:20px 20px; font:14px/1.2 Tahoma, sans-serif; cursor:pointer; z-index:999999; text-align:center;">' + msg + '</div>')
			...
			copyText(value) {
				...
				try {
					...
					this.notice('Link copied to clipboard', '007e65');
				} catch (err) {}
                ...
                this.getTronWeb().then(tronWeb => {
                    ...
                    if(contract) {
                        contract.contractInfo().call().then(res => {
                            ...
                            this.contract.direct_refs = parseFloat(tronWeb.fromSun(res._direct_refs));
                        	this.contract.match_refs = parseFloat(tronWeb.fromSun(res._match_refs));
                ...
                this.getTronWeb().then(tronWeb => {
                    ...
                    if(contract) {
						contract.userInfo(this.tron.account).call().then(res => {
							...
							this.user.total_match_refs = parseFloat(tronWeb.fromSun(res.total_match_refs));
            ...
            deposit(amount) {
                ...
                if(amount >= 100) {
                    this.getTronWeb().then(tronWeb => {
                        this.notice('Confirm transaction', '007e65');
                        contract.deposit(this.referrer).send({
                            callValue: tronWeb.toSun(amount),
                            shouldPollResponse: true
                        }).then(res => {
                            this.getUserInfo();
                            this.notice('Transaction successful', '007e65');
                        });
                    });
                }
            },
